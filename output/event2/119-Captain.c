void entrypoint_119_10() {
	start()
	switch (loadzone_temp_flags[4 /* 0x9 10 */]) {
	  case 0:
		printf("###########Good one!")
		OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 3), ('param2', 0), ('next', 125), ('param3', 39)])
		printf("######That sure looked like it felt good!\n\n\n\n######So... Why don't you try a #####fatal blow\n#####to close out our session?\n#####Sure!#####Nah.")
	  case 1:
		printf("######What's the matter, ####?\nYou don't seem your usual self.")
		OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 3), ('param2', 0), ('next', 123), ('param3', 39)])
		printf("######Stand in the center of a pack of\nenemies, and swing the Wii Remote and\nNunchuk #####horizontally at the same time#####.\nThat's how you do a real spin attack.\n######Do you want to try again?\n#####Please!#####Nah...")
	}
}

void entrypoint_119_11() {
	start()
	printf("######Great! Now I'm going to surround you\nwith logs. You need to cut them all\nwithout moving from that spot!\n\nAll you have to do is swing the\nWii Remote and Nunchuk #####horizontally\n##########at the same time#####.\n\n######Give it a shot!")
}

void entrypoint_119_12() {
	start()
	printf("######All right! When your enemy is down,\n#####target #####it with ##### and #####swing the\nWii Remote and Nunchuk at the same\ntime #####to land a #####fatal blow#####.#####\n######Do a side slash on that log. When it's\ndown, try finishing it off with this\ntechnique!")
}

void entrypoint_119_13() {
	start()
	printf("###########That's the way!")
	OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 3), ('param2', 0), ('next', 131), ('param3', 39)])
	printf("######A #####fatal blow##### is also an effective\nmove to use right after you've used\nyour #####shield #####or a well-placed #####spin attack\n#####to knock an enemy on its back.\n######Oh, but monsters don't stay down for\nlong, so you'll have to swing the\nWii Remote and Nunchuk quickly\nbefore they get back on their feet!")
}

void entrypoint_119_01() {
	start()
	switch (loadzone_temp_flags[1 /* 0x9 02 */]) {
	  case 0:
		printf("######Send my regards to the owner of\nthe Lumpy Pumpkin!")
	  case 1:
		switch (story_flags[292 /* us: 805A9AFA 0x40, jp: 805ACD7A 0x40 */]) {
		  case 0:
			switch (adventure_pouch_has(2 0x0002)) {
			  case 0:
				switch (loadzone_temp_flags[2 /* 0x9 04 */]) {
				  case 0:
					flw_57:
					OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 119), ('param2', 3), ('next', -1), ('param3', 7)])
				  case 1:
					printf("######Hey...did you happen to bring more\npiping-hot pumpkin soup?! It looks\nsooo good...\n#####Uh...here.#####...")
					switch (OrderedDict([('type', 'switch'), ('subType', 6), ('param2', 0), ('param3', 0), ('param4', 2), ('param5', 33)])) {
					  case 0:
						printf("######Oh! For me?! I mean, sorry for just\nassuming it was for me...\n\n\nWell, in that case, I'll help myself!")
						OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 2), ('param2', 20), ('next', 108), ('param3', 47)])
						OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 0), ('param2', 0), ('next', 62), ('param3', 33)])
						OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 1), ('param2', 0), ('next', 119), ('param3', 33)])
						OrderedDict([('type', 'type3'), ('subType', 2), ('param1', 1), ('param2', 100), ('next', 99), ('param3', 15)])
						OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 10), ('param2', 0), ('next', 100), ('param3', 39)])
						OrderedDict([('type', 'type3'), ('subType', 4), ('param1', 18), ('param2', 4629), ('next', 139), ('param3', 13)])
						OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 2), ('param2', 20), ('next', 102), ('param3', 48)])
						OrderedDict([('type', 'type3'), ('subType', 0), ('param1', 0), ('param2', 96), ('next', 101), ('param3', 6)])
						OrderedDict([('type', 'type3'), ('subType', 4), ('param1', 21), ('param2', 5651), ('next', 103), ('param3', 13)])
						printf("Sooo good!######")
						OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 11), ('param2', 0), ('next', 63), ('param3', 39)])
						printf("######Yeah! That hits the spot!\n\n\n\n######I tell you, morning, noon, or night,\nnothing satisfies like Pumm's yummy \npumpkin soup!")
						OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 1), ('param2', 10), ('next', 144), ('param3', 47)])
						OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 12), ('param2', 0), ('next', 143), ('param3', 39)])
						OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 2), ('param2', 0), ('next', 155), ('param3', 33)])
						OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 1), ('param2', 10), ('next', 142), ('param3', 48)])
						printf("######Thank you, ####! \nHere's a token of my appreciation.")
						OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 3), ('param2', -1), ('next', 64), ('param3', 42)])
						give_item(4 0x04);
						loadzone_temp_flags[1 /* 0x9 02 */] = true;
					  case 1:
						printf("#####...\n...#####\n\n\n######HAH! Just joking around with you!")
						loadzone_temp_flags[2 /* 0x9 04 */] = true;
					}
				}
			  case 1:
				goto flw_57
			}
		  case 1:
			switch (story_flags[291 /* us: 805A9AFA 0x20, jp: 805ACD7A 0x20 */]) {
			  case 0:
				switch (adventure_pouch_has(2 0x0002)) {
				  case 0:
					printf("######Hey! ########! Have you been\nworking hard on your swordsmanship?\n\n\n######Hmm? I smell something...\n\n\n\n######I know that earthy aroma! Isn't that\nmy favorite #####pumpkin soup#####?!\n\n\n######I asked the owner of the Lumpy\nPumpkin to bring me some, but...\n\n\nYou went out of your way to deliver\nsome to me?!\n#####That's right!#####Uh, no...")
					switch (OrderedDict([('type', 'switch'), ('subType', 6), ('param2', 0), ('param3', 0), ('param4', 2), ('param5', 23)])) {
					  case 0:
						printf("######YES! Give it to me!\n#####Here you go!#####Sorry...")
						switch (OrderedDict([('type', 'switch'), ('subType', 6), ('param2', 0), ('param3', 0), ('param4', 2), ('param5', 25)])) {
						  case 0:
							OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 2), ('param2', 20), ('next', 107), ('param3', 47)])
							OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 0), ('param2', 0), ('next', 48), ('param3', 33)])
							OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 1), ('param2', 0), ('next', 118), ('param3', 33)])
							OrderedDict([('type', 'type3'), ('subType', 2), ('param1', 1), ('param2', 100), ('next', 95), ('param3', 15)])
							OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 10), ('param2', 0), ('next', 91), ('param3', 39)])
							OrderedDict([('type', 'type3'), ('subType', 4), ('param1', 18), ('param2', 4629), ('next', 136), ('param3', 13)])
							OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 2), ('param2', 20), ('next', 93), ('param3', 48)])
							OrderedDict([('type', 'type3'), ('subType', 0), ('param1', 0), ('param2', 96), ('next', 92), ('param3', 6)])
							OrderedDict([('type', 'type3'), ('subType', 4), ('param1', 21), ('param2', 5655), ('next', 94), ('param3', 13)])
							printf("Sooo good!######")
							OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 11), ('param2', 0), ('next', 52), ('param3', 39)])
							printf("######Mmm! This is the stuff!\n\n\n\n######Pumpkin soup is best piping hot!")
							OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 1), ('param2', 10), ('next', 141), ('param3', 47)])
							OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 2), ('param2', 0), ('next', 145), ('param3', 33)])
							OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 12), ('param2', 0), ('next', 152), ('param3', 39)])
							OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 1), ('param2', 10), ('next', 140), ('param3', 48)])
							printf("######Well done! Say thanks to the owner\nfor me!")
							story_flags[292 /* us: 805A9AFA 0x40, jp: 805ACD7A 0x40 */] = true;
							loadzone_temp_flags[1 /* 0x9 02 */] = true;
							OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 3), ('param2', -1), ('next', -1), ('param3', 42)])
						  case 1:
							printf("######What?! Weren't you asked to bring\nthis to me? You think you can tease\nme like this? You DO NOT tease a man\nabout pumpkin soup!\n######If this soup isn't good anymore, you're\ngoing to have to go get me some more!\n\n\nAnd it had better be hot!")
						}
					  case 1:
						printf("######Huh... I told Pumm to bring some\nright away, so what is he doing?\n######You do not want to be around me when\nI don't get my daily pumpkin soup.")
					}
				  case 1:
					switch (adventure_pouch_has(3 0x0003)) {
					  case 0:
						printf("######Hey! ########! Have you been\nworking hard on your swordsmanship?\n\n\n######Hmm? I smell something...\n\n\n\n######I know that earthy aroma! Isn't that\nmy favorite #####pumpkin soup#####?!\n\n\n######I asked the owner of the Lumpy\nPumpkin to bring me some, but...\n\n\nYou went out of your way to deliver\nsome to me?!\n#####That's right!#####Uh, no...")
						switch (OrderedDict([('type', 'switch'), ('subType', 6), ('param2', 0), ('param3', 0), ('param4', 2), ('param5', 21)])) {
						  case 0:
							printf("######YES! Give it to me!\n#####Here you go!#####Sorry...")
							switch (OrderedDict([('type', 'switch'), ('subType', 6), ('param2', 0), ('param3', 0), ('param4', 2), ('param5', 27)])) {
							  case 0:
								OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 2), ('param2', 20), ('next', 106), ('param3', 47)])
								OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 0), ('param2', 0), ('next', 49), ('param3', 33)])
								OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 1), ('param2', 0), ('next', 117), ('param3', 33)])
								OrderedDict([('type', 'type3'), ('subType', 2), ('param1', 1), ('param2', 100), ('next', 97), ('param3', 15)])
								OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 10), ('param2', 0), ('next', 88), ('param3', 39)])
								OrderedDict([('type', 'type3'), ('subType', 4), ('param1', 18), ('param2', 4629), ('next', 133), ('param3', 13)])
								OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 2), ('param2', 20), ('next', 90), ('param3', 48)])
								OrderedDict([('type', 'type3'), ('subType', 0), ('param1', 0), ('param2', 96), ('next', 89), ('param3', 6)])
								OrderedDict([('type', 'type3'), ('subType', 4), ('param1', 16), ('param2', 4118), ('next', 147), ('param3', 13)])
								OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 11), ('param2', 0), ('next', 146), ('param3', 39)])
								printf("Ugh! This is disgusting...##### It's cold!")
								OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 1), ('param2', 10), ('next', 149), ('param3', 47)])
								OrderedDict([('type', 'type3'), ('subType', 4), ('param1', 9), ('param2', 7424), ('next', 134), ('param3', 13)])
								OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 2), ('param2', 0), ('next', 98), ('param3', 33)])
								OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 12), ('param2', 0), ('next', 150), ('param3', 39)])
								OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 1), ('param2', 10), ('next', 50), ('param3', 48)])
								printf("If there are two things I can't stand in\nthis world, it's cold soup and slackers.\nNow bring me another one--and make\nit hot!")
								OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 3), ('param2', -1), ('next', -1), ('param3', 42)])
							  case 1:
								printf("######What?! Weren't you asked to bring\nthis to me? You think you can tease\nme like this? You DO NOT tease a man\nabout pumpkin soup!\n######If this soup isn't good anymore, you're\ngoing to have to go get me some more!\n\n\nAnd it had better be hot!")
							}
						  case 1:
							printf("######Huh... I told Pumm to bring some\nright away, so what is he doing?\n######You do not want to be around me when\nI don't get my daily pumpkin soup.")
						}
					  case 1:
						OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 119), ('param2', 3), ('next', -1), ('param3', 7)])
					}
				}
			  case 1:
				OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 119), ('param2', 3), ('next', -1), ('param3', 7)])
			}
		}
	}
}

void entrypoint_119_02() {
	start()
	OrderedDict([('type', 'type3'), ('subType', 0), ('param1', 0), ('param2', 11), ('next', 25), ('param3', 32)])
	printf("######Hey! #############!")
	OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 11), ('param2', 0), ('next', 72), ('param3', 16)])
	OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 1), ('param2', 0), ('next', 28), ('param3', 39)])
	printf("######You can't take swords outside of the\nSparring Hall.\n\n\nWhat do you think you're doing\nsneaking out with that?!")
	OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 2), ('param2', 0), ('next', 29), ('param3', 39)])
	OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 3), ('param2', 11), ('next', 32), ('param3', 33)])
	OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 100), ('param2', 11), ('next', 26), ('param3', 17)])
	printf("######What? ####(Your Loftwing is missing?\n####(You want to go look for it?\n\n\nWell I guess I could make an exception\njust this once. ######But remember that\nSkyloft is a peaceful place. Only\nknights carry swords!\nSo I don't want to see you swinging\nit around like crazy out there!\nI don't want to get in a ton of trouble\nover this, so here's the deal...\nMake sure you get plenty of practice\non these logs first, and once you get\noutside with that sword, only use it\nwhen you absolutely have to, got it?")
	OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 3), ('param2', -1), ('next', 27), ('param3', 42)])
	story_flags[199 /* us: 805A9AEF 0x02, jp: 805ACD6F 0x02 */] = true;
	scene_flags[0 'Skyloft'][56 /* 0x6 01 */] = true;
}

void entrypoint_119_03() {
	start()
	switch (adventure_pouch_has(9 0x0009)) {
	  case 0:
		switch (scene_flags[60 /* 0x6 10 */]) {
		  case 0:
			printf("######Oh? Ready to talk more about the\n#####secrets of defense#####?\n#####Practical stuff.#####Basics!#####No, thanks.")
			switch (choice(3)) {
			  case 0:
				printf("#####Practical stuff#####? OK!\n\n\n\n######If you ready your shield right before an\nenemy attacks you, the force from your\nshield arm will #####repel #####the enemy's \nattack. \n######This technique is called a #####shield bash#####,\nand it will make the enemy recoil,\ngiving you an opening in which to\nattack.\n######This technique is very useful against\nenemies who throw weapons at you or\njust refuse to give you an opening\nfor attack.")
				flw_21:
				printf("######In a real battle, pulling off a defensive\nshield maneuver requires perfect\ntiming. Believe me, you really don't\nwant to miss.\n######Why don't you put what you learned\nto use with that #####defense-practice##### #####log#####?")
			  case 1:
				printf("###########Give your Nunchuk a shake #####to take\nyour #####shield##### out. Simple stuff, I know,\nbut you need to review the basics!\n\n######To lower your shield, #####swing your sword\nwith the Wii Remote#####, or press ##### to put\nyour sword and shield away entirely.\n\n######Just be careful when trying to attack\nwhile on the defensive. Every time you \n#####swing your sword#####, you move your shield\nout of the way, #####lowering your guard#####!")
				goto flw_21
			  case 2:
				printf("######I see... Well, trying it out for yourself\nis the best way to learn. Why don't\nyou practice on that defense log?")
			}
		  case 1:
			switch (scene_flags[59 /* 0x6 08 */]) {
			  case 0:
				printf("######Impressive, ####! That's\na nice shield you have there.\n######Well then, it's time I taught you the\n#####art of defense#####!\nWe'll start with #####basic training#####!")
				flw_15:
				printf("###########Give your Nunchuk a shake #####to take\nyour #####shield##### out. Simple stuff, I know,\nbut you need to review the basics!\n\n######To lower your shield, #####swing your sword\nwith the Wii Remote#####, or press ##### to put\nyour sword and shield away entirely.\n\n######Just be careful when trying to attack\nwhile on the defensive. Every time you \n#####swing your sword#####, you move your shield\nout of the way, #####lowering your guard#####!")
				printf("######Why don't you try that first!")
				scene_flags[0 'Skyloft'][60 /* 0x6 10 */] = true;
			  case 1:
				printf("##############... The headmaster\ntold me about Zelda. Poor girl...\nI'm worried about her...\n\nNow, I don't know all the details of\nwhat's going on here...\n\n\nBut that sword you're carrying isn't\nfrom Skyloft. It's an #####ancient artifact#####\nthat can only be carried by the chosen\none.\nAnd since you happen to be the one\ncarrying it, I guess I can figure out\na lot of the rest of this crazy story...")
				printf("######You know, ####...\nI just wish there was more I could do\nfor you...\n\nBut I can offer this as a parting gift...\n######Get ready to learn some new moves!\nIt's time to learn the #####secrets of\ndefense#####.\n######First, we should start with the #####basics#####!")
				goto flw_15
			}
		}
	  case 1:
		switch (scene_flags[59 /* 0x6 08 */]) {
		  case 0:
			printf("######The art of the shield is the art of\ndefense. You don't have a chance of\nwinning a battle if your attack and\ndefense aren't balanced.\n######You can get a #####shield #####at the##### bazaar#####.\nAlways practice with a shield in your\nhand. It will lead you to the way of\nthe sword!\n######I'm telling you all this for your\nbenefit, so pay attention!")
		  case 1:
			switch (story_flags[30 /* us: 805A9ADA 0x40, jp: 805ACD5A 0x40 */]) {
			  case 0:
				printf("##############... The headmaster\ntold me about Zelda. Poor girl...\nI'm worried about her...\n\nNow, I don't know all the details of\nwhat's going on here...\n\n\nBut that sword you're carrying isn't\nfrom Skyloft. It's an #####ancient artifact#####\nthat can only be carried by the chosen\none.\nAnd since you happen to be the one\ncarrying it, I guess I can figure out\na lot of the rest of this crazy story...")
				printf("######I wish there was something else I could\ndo for you, ####...\nWait, maybe there is... Yes! Get a\n#####shield #####and come back here!\n######Then I'll be able to teach you the\n#####secrets of defense#####! It's the least\nI can do for you.")
				scene_flags[0 'Skyloft'][59 /* 0x6 08 */] = true;
			  case 1:
				switch (scene_flags[2 /* 0x1 04 */]) {
				  case 0:
					printf("######Oh yeah? You want to learn about how\nto use a sword?\n#####Yes, please. #####No, thanks.")
					switch (OrderedDict([('type', 'switch'), ('subType', 6), ('param2', 0), ('param3', 0), ('param4', 2), ('param5', 0)])) {
					  case 0:
						printf("######OK! #####Just swing the Wii Remote #####to swing\nyour sword!\n\n\nSwing the Wii Remote vertically to do a\n#####vertical slash#####, and swing it sideways to\ndo a #####side slash#####.\n\nThrust the Wii Remote forward to do a\n#####thrust#####!\n\n\n######And then there's the #####spin attack#####,\na more advanced technique. To do that,\nswing the Wii Remote and Nunchuk\ntogether in a sideways motion.\n######If you remember these moves, you can\nhandle yourself in any fight.\n\n\n######Just remember the fundamentals!\nAlways press##### ########## #####to target##### an enemy\nas soon as you see it.#####")
						flw_7:
						printf("######Try getting in some good practice on\nthose #####logs#####!\n\n\n######If the sword doesn't respond quite the\nway you want it to, try big #####vertical\n##########overhead practice##### ###############swings #####in front of\nyou, then get back to the action.\n######Once you've sliced up those logs, I'll\nset it up so you can try out your\n#####big move#####.")
					  case 1:
						printf("######That's right. You're good with a sword.\n\n\n\n######But practice is everything! You don't\nbecome a sword master sitting on \nyour rear end.\n\n######You've got to practice those\nfundamentals, like using ########## to target\nenemies#####. It's not glamorous, but that's\na swordsman's bread and butter.#####")
						goto flw_7
					}
				  case 1:
					printf("######Hey! It's ####!\n\n\n\n######So you still want to get in some sword\npractice, even though today is the Wing\nCeremony? I'm impressed.\n\nThe sword is in the back room.\nGo on back there and get it.\n\n\nI'll be here for a while, so if there's\nanything you don't understand about \nhandling a sword, go ahead and ask.")
				}
			}
		}
	}
}

void entrypoint_119_04() {
	start()
	printf("######That's a log for practicing your\n#####vertical slash#####. Hold the Wii Remote\nstraight up, and then swing it straight\ndown to make a clean cut!")
}

void entrypoint_119_05() {
	start()
	printf("######That's a log for practicing your\n#####side slash#####. Hold the Wii Remote\nhorizontally, and then slash it to the\nside to make a clean cut!")
}

void entrypoint_119_06() {
	start()
	printf("######That log is set up for practicing your\n#####diagonal slash#####. Swing the Wii Remote\ndiagonally to cut right along the gash\nin the log.")
}

void entrypoint_119_07() {
	start()
	printf("######That's a log used for practicing your\n#####thrust#####. Sharply move the Wii Remote\nforward to thrust your sword.")
}

void entrypoint_119_08() {
	start()
	switch (adventure_pouch_has(0 0x0000)) {
	  case 0:
		printf("######That log is used to practice defense.\n\n\n\n######Hit the log with your sword, and then\n#####sharply move the Nunchuk forward #####to\nexecute a #####shield bash #####when it swings\nback at you.")
	  case 1:
		switch (adventure_pouch_has(9 0x0009)) {
		  case 0:
			printf("######That log is used to practice #####defense#####.\nYou don't have a #####shield##### equipped,\nthough, so don't mess with it, or it'll\nsend you flying!")
		  case 1:
			printf("######That log there is for practicing\n#####defense#####. You don't have a #####shield##### yet, so\ndon't mess with it, or it'll send you\nflying!")
		}
	}
}

void entrypoint_119_09() {
	start()
	printf("####\n#That's right, ####!")
	OrderedDict([('type', 'type3'), ('subType', 1), ('param1', 3), ('param2', 0), ('next', 110), ('param3', 39)])
	printf("######You really are a strong swordsman. \nEasy to teach too. You make this\ninstructor proud!\n\n######Are you ready to try a #####spin attack#####?\n#####OK!#####Not yet.")
}

